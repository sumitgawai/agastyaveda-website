...
function createApp() {
  ...
  app.get("/api/config", (req, res) => res.json({ clerkEnabled: config.clerkEnabled && !config.localAuthBypass, clerkPublishableKey: config.clerkPublishableKey || null, googleMapsApiKey: config.googleMapsApiKey || null, razorpayEnabled: config.razorpayEnabled }));
  ...
  app.post("/api/appointments", requireAuth, async (req, res, next) => {
    try {
      ...
      if (!date || !time || !email) return res.status(400).json({ error: "Date, time and email are required." });
      ...
      const appointment = await appointments.insert({ date, time, email, userId: req.auth.userId, fee: config.consultationFee, status: "REQUESTED", createdAt: new Date().toISOString() });
  ...
  app.post("/api/orders", requireAuth, async (req, res, next) => {
    try {
      ...
      if (typeof address !== "object" || !address.name || !address.line1 || !address.city || !address.state || !address.postalCode || !address.latitude || !address.longitude) return res.status(400).json({ error: "Complete delivery details and map location are required." });
      const products = await store("products").findMany({ published: true });
